    ...
    <div className="logs-container" style={{ marginTop: '40px', border: '1px solid #3b82f6' }}>
      ...
      {hasCache && (
        <div style={{ marginBottom: '16px' }}>
          ...
          >
            ⬇ Export measurements.json (for sweep script)
          </button>
          <span style={{ marginLeft: '12px', color: '#64748b', fontSize: '0.8rem' }}>
            Then run: <code style={{ background: '#1e293b', padding: '2px 6px', borderRadius: '4px' }}>node sweep.mjs</code> in the project root
          </span>
        </div>
      )}
              ...
              <tbody>
                {testResults.map((result, idx) => (
                  <tr key={idx} style={{ background: idx % 2 === 0 ? '#0f172a' : '#1e293b' }}>
                    ...
                    <td style={{ padding: '10px 12px', borderBottom: '1px solid #334155', textTransform: 'capitalize' }}>
                      <div style={{ fontWeight: result.match ? 'normal' : 'bold' }}>{result.predicted}</div>
                      {!result.match && result.process && (
                        <div 
                          style={{ 
                            fontSize: '0.7rem', 
                            color: '#94a3b8', 
                          ...
                          title={result.process.join('\n')}
                        >
                          {result.process.find(p => p.includes('FAIL')) || result.process[0]}
                        </div>
                      )}
                    </td>
